...
public class Basics {
    ...
    public TreeNode trimBST(TreeNode root, int low, int high) {
        ...
        if (root.val >= low && root.val <= high) {
            // valid tree : go fotr left and tree tree trimming
            root.left = trimBST(root.left, low, high);
            root.right = trimBST(root.right, low, high);
        } else if (root.val < low) {
            // since root is smaller than low and it is BST, so left subtree will even be
            // smaller
            // so no profit of calling the left, and can't even include the root as it
            // voilates the condition
