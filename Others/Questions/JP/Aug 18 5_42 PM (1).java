...
//   ********************************** Approach 1 : Using Integer to count the no of threads accessing the node to be locked ********
    ...
    private static boolean lock(String s, int uid){
        ...
        while (par != null) {
            par.desCount++;
            par.lockedChildren.add(curr.name);
            par = par.parent;
        }
...
//       	//entry section
//       	curr.isLocked++; // race variable
//       // we already make it as locked : help in finding no of threads and if at any point we found the conditions are not met we simply remove the lock
      
//         if(curr.isLocked > 1 ) {
//           curr.isLocked--;  //exit condin
//           return false; 
//         }
...
//       	// desendants
//         if(curr.desCount > 0){
//           curr.isLocked = 0; // can't be locked now
//           curr.lockedBy = -1;
//            return false;
//         }
...
//        //ancestors
//         Node par = curr.parent;
//       	boolean flag = false; //check for ancestors validity : help in reverting back descount operation
//       synchronized(this){
//           while(par != null){
//             par.desCount++;
//             if( par.isLocked == 1){ //corrected as par.desc can be > 1 (locking siblings )
//               flag = true;
//               break;
//             }
//             par.lockedChildren.add(curr.name); // all ancestors will have the information of the locked desendants
//             par = par.parent; //update parent
//           }
