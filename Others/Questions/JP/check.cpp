...
bool checkDesc(Tree *node, int id)
{
    ...
    for (int i = 0; i < (node->child).size(); i++)
    {
        ...
        {
            if (((node->child)[i])->uid != id)
                return false;
            anyLockNode = true;
        }
        else if (((node->child)[i])->desCnt > 0)
        {
            if (checkDesc((node->child)[i], id) == false)
                return false;
        }
    }
    return true;
...
int main()
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        if (i == 0)
        {
            Tree *newNode = new Tree(names[i], root);
        ...
        {
            Tree *newNode = new Tree(names[i], nodes[(i - 1) / M]);
            nodes.push_back(newNode);
            mp[s] = newNode;
        }
        // names.push_back(s);
    ...
    for (int i = 0; i < N; i++)
    {
        for (int j = 1; j <= M; j++)
        {
            int cld = M * i + j;
            if (cld < N)
                (nodes[i]->child).push_back(nodes[cld]);
        }
    }
