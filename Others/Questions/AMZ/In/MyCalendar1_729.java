...
class MyCalendar {
    ...
    private TreeMap<Integer, Integer> map; // TreeMap container clas to use floorkey / lowerkey as Map interface doesb't
                                           // provides that
    ...
    // Approach 2:
    {
        ...
        // get next (greater) meeting , if that's start time(key) is greater that mine
        // end time : overlapping
    }
    ...
    private boolean appraoch2(int currStart, int currEnd) {
        ...
        if (previousEnd > currStart || currEnd > nextStart)
            return false;

        // all are valid;
        map.put(currStart, currEnd); // log n
    ...
    // Approach 1 :
    {
        ...
        // s1 s2 e2 e1
        // so basicallly we can say one pair(s,e) should complete each other without any
    ...
    private boolean appraoch1(int start, int end) {
        ...
        {
            // The point at which you call entrySet() there is no (significant) work to do
            // beyond handing back a reference, so it's constant time. The map maintains
            // sorted behavior as things are added to / removed from the TreeMap.
