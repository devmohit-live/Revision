...
class TheSkylineProblem_218 {
    public List<List<Integer>> getSkyline(int[][] B) {
        ...
        // reason for not choosing priority queue is removal of given item other than
        // root can be costly O(n)
        ...
        for (int[] h : H) {
            ...
            if (h[1] < 0)
                ...
                if (map.get(h[1]) == 0)
                    map.remove(h[1]); // could have been costly for priority queue
            }
