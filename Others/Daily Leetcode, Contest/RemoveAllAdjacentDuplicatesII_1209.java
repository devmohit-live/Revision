public class RemoveAllAdjacentDuplicatesII_1209 {
    ...
    private String usingPairClasAndStack(String s, int k) {
        class Pair {
            char ch;
            int freq;
    ...
    private String usingStringBuilderOnly(String s, int k) {
        ...
        for (char ch : s.toCharArray()) {
            ...
            count[lastIdx] = 1
                    + ((lastIdx > 0 && sb.charAt(lastIdx) == sb.charAt(lastIdx - 1)) ? count[lastIdx - 1] : 0);
            if (count[lastIdx] >= k)
