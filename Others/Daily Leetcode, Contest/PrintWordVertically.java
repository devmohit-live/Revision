...
// LC : 1324 : https://leetcode.com/problems/print-words-vertically/discuss/1640833/Java-Solution-or-String-builder-or-Commented-Code-for-better-understanding
class PrintWordVertically {
    public List<String> printVertically(String s) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < k; j++) {
                if (j < len)
                    sbs[j].append(word.charAt(j));
                else
                    sbs[j].append(" "); // for the words which don't have max length
                pointers[j]++;
            }
        }
        ...
        for (int i = 0; i < k; i++) {
            for (int j = sbs[i].length() - 1; j >= 0; j--) {
                ...
                if (sbs[i].charAt(j) != ' ')
                    ...
                    sbs[i].deleteCharAt(j);
            }
            if (sbs[i].length() == 0)
                sbs[i].append(" "); // words with all spaces
