...
//         POSTORDER:
        ...
        // how do we know the range? => since we already know the number of leftsubtree and right subtree elements from the INORDER data we can use that
    ...
    private TreeNode buildTree(int[] in, int[] post,int insi, int inei, int posi, int poei) {
        if(posi>poei || insi>inei) return null;
        ...
        // left-subtree = psi-> psi+totalleftel-1, right-subtree=>totalleftel->pei-1 (pei is root)
